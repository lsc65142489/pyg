  ...
  */
  ...
  function eventList(params) {
    ...
    $(".left_menu ").on("tap", "li", function () {
      // 当前点击的按钮显示高亮，排他
      $(this).addClass("active").siblings().removeClass("active"); // 获取被点击到的li标签的索引 方法 $(this).index();

      var index = $(this).index(); // 调用渲染数据右边数据的方法
      // console.log($(this))
  ...
  function categories() {
    $.get("http://api.pyg.ak48.xyz/api/public/v1/categories", function (result) {
      if (result.meta.status == 200) {
        // console.log(result)
        // 获取要渲染的数据
        CateDatas = result.data; // 调用渲染左边的方法

        renderLeft(); // 调用渲染右边的方法
  ...
  function renderLeft() {
    ...
    for (var i = 0; i < CateDatas.length; i++) {
      var tmpHtml = "\n            <li class=\"".concat(i == 0 ? "active" : "", "\">\n            ").concat(CateDatas[i].cat_name, "</li>\n            ");
      leftHtml += tmpHtml;
    }
  ...
  function renderRight(index) {
    ...
    var rightData = item2Obj.children; // 渲染数据

    var rightHtml = template("rightTp1", {
      arr: rightData
    });
